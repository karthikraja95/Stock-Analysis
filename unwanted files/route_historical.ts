...
async function fetchFinancialData(ticker: string) {
  try {
    ...
    const { price, summaryDetail, financialData, defaultKeyStatistics } = quoteSummary;
    ...
    return {
      MarketCapitalization: price.marketCap 
        ...
        : (price.regularMarketPrice && price.sharesOutstanding 
          ? `$${((price.regularMarketPrice * price.sharesOutstanding) / 1e9).toFixed(2)}B` 
          : 'N/A'),
      PERatio: financialData.forwardPE || defaultKeyStatistics.trailingPE || 'N/A',
